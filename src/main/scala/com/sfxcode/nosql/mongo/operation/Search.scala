...
abstract class Search[A]()(implicit ct: ClassTag[A]) extends Base[A] {
  ...
  def find(filter: Bson = Document(),
           sort: Bson = Document(),
           projection: Bson = Document()): FindObservable[A] =
  ...
  def distinct[S <: Any](
      fieldName: String,
      filter: Bson = Document()): DistinctObservable[BsonValue] = {
    coll.distinct[BsonValue](fieldName, filter)
  }

  def distinctResult[S <: Any](fieldName: String,
                               filter: Bson = Document()): Seq[S] = {
