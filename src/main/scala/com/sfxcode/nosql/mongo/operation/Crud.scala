...
abstract class Crud[A]()(implicit ct: ClassTag[A]) extends Search[A] {
  ...
  def insertMany(values: Seq[A],
                 options: InsertManyOptions): Observable[Completed] =
  ...
  def replaceOne(value: A,
                 options: ReplaceOptions): Observable[UpdateResult] = {
    val document = Converter.toDocument(value)
    val oid = document.get("_id").get
  ...
  def updateOne(filter: Bson,
                update: Bson,
                options: UpdateOptions): Observable[UpdateResult] =
